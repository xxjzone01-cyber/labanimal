...
beforeAll(async () => {
  ...
  db.run(`CREATE TABLE animals (id TEXT PRIMARY KEY, lab_id TEXT, internal_id TEXT, species TEXT, sex TEXT, status TEXT, quarantine_status TEXT)`);
  db.run(`CREATE TABLE rooms (id TEXT PRIMARY KEY, lab_id TEXT, name TEXT)`);
  db.run(`CREATE TABLE protocols (id TEXT PRIMARY KEY, lab_id TEXT, title TEXT, pi_name TEXT, status TEXT)`);
  ...
  db.run(`INSERT INTO animals VALUES ('a2', 'lab-1', 'M-002', 'mouse', 'female', 'active', 'none')`);
...
describe('migrate', () => {
  ...
  it('源文件不存在时抛出错误', async () => {
    await expect(
      migrate({ source: '/nonexistent/path.db', dryRun: true })
  ...
  it('无 DATABASE_URL 时抛出错误', async () => {
    ...
    await expect(
      migrate({ source: dbPath })
    ).rejects.toThrow('未指定目标数据库');
  ...
  it('dry-run 统计各表记录数', async () => {
    ...
    const tableStats = Object.fromEntries(
      results.map(r => [r.table, r.imported])
    );
