...
export function mapRow(
  ...
  for (const mapping of mappings) {
    ...
    if (value !== undefined) {
      result[mapping.target] = mapping.transform ? mapping.transform(value) : value;
    } else if (!(mapping.target in result)) {
      // 首次遇到该目标字段且源不存在，设为 undefined
      result[mapping.target] = mapping.transform ? mapping.transform(value) : undefined;
    }
  }
 ...
 */
export function mapEnum(value: unknown, allowed: readonly string[], fallback: string): string {
