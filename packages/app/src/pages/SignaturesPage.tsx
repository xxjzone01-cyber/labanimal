    ...
    <div>
      ...
      {showSign && (
        <div className="bg-white rounded-xl p-6 border border-gray-200 mb-6">
          ...
          <p className="text-xs text-gray-500 mb-4">21 CFR Part 11 compliant — review carefully before signing</p>
          ...
          <form onSubmit={handleSign}>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
              ...
              <div className="text-sm text-amber-700 space-y-1">
                <p>I, <strong>the authenticated user</strong>, hereby sign:</p>
                <p><strong>Entity:</strong> {signForm.entityType} / {signForm.entityId || '...'}</p>
                <p><strong>Meaning:</strong> I have {signForm.meaning} this document</p>
                {signForm.notes && <p><strong>Notes:</strong> {signForm.notes}</p>}
                <p className="text-xs text-amber-600 mt-2">
                  This signature will be hashed with SHA-256 and recorded with timestamp and IP address per 21 CFR Part 11.
                </p>
              </div>
            </div>
      ...
      {lastSigned && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6">
          <div className="flex items-center justify-between mb-2">
            <span className="text-sm font-medium text-green-800">Signature Created Successfully</span>
            <button onClick={() => setLastSigned(null)} className="text-green-600 text-xs">Dismiss</button>
          </div>
          <div className="grid grid-cols-2 gap-2 text-xs text-green-700">
            <div><strong>Signature ID:</strong> {lastSigned.id}</div>
            <div><strong>Signed By:</strong> {lastSigned.user.name} ({lastSigned.user.email})</div>
            <div><strong>Entity:</strong> {lastSigned.entityType} / {lastSigned.entityId}</div>
            <div><strong>Meaning:</strong> {lastSigned.meaning}</div>
            <div><strong>Timestamp:</strong> {new Date(lastSigned.signedAt).toLocaleString()}</div>
            <div><strong>IP Address:</strong> {lastSigned.ipAddress || 'N/A'}</div>
            <div className="col-span-2"><strong>SHA-256 Hash:</strong> <code className="text-xs break-all">{lastSigned.signatureHash}</code></div>
          </div>
        </div>
      )}
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500">No signatures</td></tr>
            ) : (
              signatures.map((s) => (
                <tr key={s.id} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3 text-sm">{s.user.name}</td>
                  <td className="px-4 py-3 text-sm">{s.entityType}/{s.entityId}</td>
                  ...
                  <td className="px-4 py-3 text-xs font-mono text-gray-500 max-w-32 truncate" title={s.signatureHash}>
                    {s.signatureHash.slice(0, 16)}...
                  </td>
                  <td className="px-4 py-3">
                    {verification[s.id] !== undefined && (
                      <span className={`inline-block px-2 py-1 text-xs rounded-full ${verification[s.id] ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                        {verification[s.id] ? 'Valid' : 'Invalid'}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <button onClick={() => handleVerify(s.id)} className="text-blue-600 hover:text-blue-800 text-sm">
                      Verify
                    </button>
                  </td>
                </tr>
