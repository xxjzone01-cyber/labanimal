...
export function TrainingsPage() {
  ...
  async function handleAdd(e: React.FormEvent) {
    ...
    try {
      ...
      setShowAdd(false);
      setNewTraining({ userId: '', type: 'aalas_lat', certificationNumber: '', issuedBy: '', expirationDate: '' });
      loadTrainings();
    } catch (err: any) {
      setError(err.message);
    }
        ...
        <form onSubmit={handleAdd} className="bg-white rounded-xl p-6 border border-gray-200 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input value={newTraining.userId} onChange={(e) => setNewTraining({ ...newTraining, userId: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Type</label>
              <select value={newTraining.type} onChange={(e) => setNewTraining({ ...newTraining, type: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm">
            ...
            <div>
              ...
              <input value={newTraining.certificationNumber} onChange={(e) => setNewTraining({ ...newTraining, certificationNumber: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Expiration Date</label>
              <input type="date" value={newTraining.expirationDate} onChange={(e) => setNewTraining({ ...newTraining, expirationDate: e.target.value })} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
            </div>
          </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={5} className="px-4 py-8 text-center text-gray-500">No training records</td></tr>
            ) : (
              trainings.map((t) => (
                <tr key={t.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm">{t.expirationDate ? new Date(t.expirationDate).toLocaleDateString() : '—'}</td>
